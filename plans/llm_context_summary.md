...
## Current Live State
- `plans/work_queue.csv` is down to one active recent-accounting task, `ACC-RA-033`, plus the formal `EXT-*` staged-external lane.
...
- Staged external execution still depends on operators, even though the request specs are now self-describing and queued formally.
